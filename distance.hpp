...
class ANNS{
    ...
    public:
        ...
        void IVF_knn(const std::map<int, std::vector<std::vector<float>>>& IVF, int knn_cluster){
            // Find closest clusters to each query
            // Store results in closest_clusters: [[{0, d0}...{k, dk}], [{0, d0}...{k, dk}],...,[{0, d0}...{0, dk}]]  
            //                                            query 1              query 2      ...        query n
            for(const std::vector<float>& q: query_vecs){
                int i = 0;
                std::vector<std::pair<int, double>> result_q;
                for(const auto& data : IVF){
                    std::vector<float> cluster = data.second[0];
            ...
            // Perform KNN on the closest clusters
            // Loop over each query
            for(int i = 0; i < static_cast<int>(closest_clusters.size()); ++i){
                int key = 0;
                ...
                for(int j = 0; j < static_cast<int>(closest_clusters[i].size()); ++j){
                    ...
                    const std::vector<std::vector<float>>& cluster_data = IVF.at(cluster_id);
                    ...
                    for(int k = 1; k < static_cast<int>(cluster_data.size()); ++k){
                        const std::vector<float>& point_vec = cluster_data[k];
