...
class KMeans{
    private:
        ...
        std::vector<std::vector<std::vector<float>>> kmeans_results;
        std::map<int, std::vector<std::vector<float>>> IVF;
    ...
    public:
        ...
        void assign_clusters(std::vector<std::vector<float>> data, std::vector<std::vector<float>> clusters_list){
            // Assign each data point to the nearest cluster
            std::vector<std::vector<std::vector<float>>> kmeans_list;

            // Push clusters to kmeans_list
            for(std::vector<float> cluster: clusters_list){
                std::vector<std::vector<float>> cluster_i = {cluster};
                kmeans_list.push_back(cluster_i);
            }

            // Calculate distance from each cluster to data point
            // Assign each data point to the nearest cluster  
            for(const std::vector<float>& elem: data){
                ...
                kmeans_list[min_cluster_id].push_back(elem);
            }
        ...
        void bisect_kmeans(const int& max_num_clusters){ 
            ...
            assign_clusters(base_data, initial_clusters);
            while(true){
                ...
                if(static_cast<int>(kmeans_results.size()) >= max_num_clusters){
                    ...
                    std::vector<std::vector<float>> cluster_data = kmeans_results[largest_cluster_id];
        ...
        void build_index(const int& max_num_clusters){
            ...
            // Build IVF index
            for(int i = 0; i < static_cast<int>(kmeans_results.size()); ++i){
                std::vector<std::vector<float>> data_in_cluster = kmeans_results[i];
                IVF[i] = data_in_cluster;
            }
        }

        std::vector<std::vector<std::vector<float>>> get_kmeans_clusters(){
            return kmeans_results;
        }

        std::map<int, std::vector<std::vector<float>>> get_IVF(){
            return IVF;
        }
